...
public class GameBoardTests
{
    ...
    public void ExplorationTest()
    {
        ...
        gameBoard.SetRow(0, new Token[] { new(TokenType.Stamina), new(TokenType.Joy), new(TokenType.Talent), new(TokenType.Sentiment), new(TokenType.Flirtation), new(TokenType.Passion) });
        gameBoard.SetRow(1, new Token[] { new(TokenType.Talent), new(TokenType.Romance), new(TokenType.Romance), new(TokenType.Sexuality), new(TokenType.Flirtation), new(TokenType.Flirtation) });
        gameBoard.SetRow(2, new Token[] { new(TokenType.Romance), new(TokenType.Passion), new(TokenType.Talent), new(TokenType.Romance), new(TokenType.Sexuality), new(TokenType.Flirtation) });
        gameBoard.SetRow(3, new Token[] { new(TokenType.Joy), new(TokenType.Stamina), new(TokenType.Talent), new(TokenType.Passion), new(TokenType.Sentiment), new(TokenType.Talent) });

        if (gameBoard.HasMatch())
            Assert.Fail("The test must start without a match");

        var move = gameBoard.FindBestMoveConsideringFuture(5);
    }
}
